...
/**This class implements the generic interface for objects of type CustomerQuery and Insurer
 * It overrides checkCriteriaAndReturnQuote for the specific criteria logic provided in assessment
 * @author Madhumita
 *
 */
@Component
public class CriteriaInsurerImpl implements ICriteria<CustomerQuery,Insurer>{
	...
	public static final Logger logger = LoggerFactory.getLogger(CriteriaInsurerImpl.class);
	...
	@Override
	public Map<String,Double> checkCriteriaAndReturnQuote(CustomerQuery query,List<Insurer> insurers){
		...
		if(mustCriteria!=null && mustCriteria.size()>0){
				...
				if(mustCriteria.contains(config.POSTCODE) && mustCriteria.contains(ApplicationConfiguration.OCCUPATION)){
				
					insurersWithExcludedPostCodesAndOccupations = insurerRepository.findByExcludedOccupationsAndExcludedPostCodes(query.getPostCode(), query.getOccupation());
					excludedInsurers.addAll(insurersWithExcludedPostCodesAndOccupations);
				}
				else if(mustCriteria.contains(config.OCCUPATION)){
								
				
					insurersWithExcludedOccupations = insurerRepository.findByExcludedOccupations(query.getOccupation());
					excludedInsurers.addAll(insurersWithExcludedOccupations);
				}
				else if(mustCriteria.contains(config.POSTCODE)){
								
					insurersWithExcludedPostCodes = insurerRepository.findByExcludedPostCodes(query.getPostCode());
		...
		if(shouldCriteria!=null && shouldCriteria.size()>0){
			...
			if(shouldCriteria.contains(ApplicationConfiguration.LESS_THAN_MINIMUM_TURNOVER)){
				logger.debug("shouldCriteria has MINIMUM_TURNOVER ");
				excludedInsurers.addAll(insurerRepository.findByMinimumTurnOverGreaterThan(query.getTurnOver()));
			}
		}
